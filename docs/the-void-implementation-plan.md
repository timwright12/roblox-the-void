...
**Studio playtest status (in progress):** confirmed via direct debugging that `_nextState` and path computation both work correctly in Studio (state transitions Idle→Chasing fire at the right distance, `PathfindingService` returns real multi-waypoint paths, `Enum.PathStatus.Success`). Two real issues found and only partially resolved so far:
- The Phase 4 Toolbox robot/zombie models (`ScoutRobot`/`GruntRobot`/`HeavyRobot`/`ZombieBase`) had their PrimaryPart (`Torso`/`HumanoidRootPart`) `Anchored = true` by default, which silently prevents any Humanoid-driven movement — `MoveTo` calls succeed with no error, but the anchored part physically cannot move. **Fix:** uncheck `Anchored` on each model's PrimaryPart in Studio (not Rojo-managed, since these are Toolbox models — see Section 9's own model-sourcing note).
- After unanchoring, only the PrimaryPart moves — the rest of each model's parts (limbs, head) stay behind, meaning these specific Toolbox models likely lack proper Motor6D rig joints connecting them to the Humanoid. This is a model-quality issue with the chosen assets, not a code bug in `EnemyAI`. Not yet resolved — needs either better-rigged replacement models from the Toolbox, or accepting the visual glitch as a placeholder (same spirit as the code-built placeholder cube before Phase 4).
