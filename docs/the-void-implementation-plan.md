...
| `ShopUIController` | Client | Renders shop menu from `WeaponTable`, sends purchase requests, reflects server-confirmed state (don't optimistically show a purchase as successful before server confirms). Opens via the "B" key or an always-visible on-screen button — the button exists because "B" has no equivalent on touch-only devices (e.g. iPad), which would otherwise make the shop unreachable there. |
...
- Enters `Chasing` if a player is within `AGGRO_RADIUS_STUDS` studs (comfortably beyond `ENEMY_MELEE_RANGE = 6`, so the enemy visibly closes distance before it can attack, and within typical ranged-weapon range — Crossbow 30, Longbow 45 — so a player who never melees still gets engaged). Widened from an initial 25 to **100** after playtesting found enemies too easy to avoid entirely.
...
While `Chasing`, `EnemyAI` computes a path via `PathfindingService:CreatePath` (default `AgentParameters`, no custom `AgentRadius`/`Costs` yet — a placeholder starting point, not tuned) to the target player's current `HumanoidRootPart.Position`, and walks the enemy's `Humanoid` along the returned waypoints via `MoveTo`. Because the player keeps moving, the path is recomputed on a shorter interval than the aggro check itself (`PATH_RECOMPUTE_INTERVAL_SECONDS`, tightened from an initial 1s to **0.25s** — 1s recomputes made chasing feel sluggish against a moving player) rather than once — a stale path would walk the enemy to where the player *was*. `Path.Blocked` also triggers an immediate recompute rather than waiting for the next interval, so an enemy doesn't stand still against a moved obstacle for up to a full second.

`EnemyAI._shouldRetarget(currentTarget, newTarget)` guards each recompute: a new waypoint within `RETARGET_TOLERANCE_STUDS` (3 studs) of the currently-targeted point does not reissue `Humanoid:MoveTo()`. Without this, the faster 0.25s recompute interval would restart the walk toward a near-identical point up to 4 times a second, which reads as choppy/rigid rather than a smooth chase — a real symptom found in Studio playtesting. Unit-tested in `EnemyAI.spec.luau` as a pure function.
...
**Studio playtest status (in progress):** confirmed via direct debugging that `_nextState` and path computation both work correctly in Studio (state transitions Idle→Chasing fire at the right distance, `PathfindingService` returns real multi-waypoint paths, `Enum.PathStatus.Success`). Root cause of "only the torso moves" turned out to be per-model, not a single bug — diagnosed by dumping each model's `BasePart`/`Weld`/`WeldConstraint` descendants (name, position, `Anchored`, and weld `Part0`/`Part1`) via the Command Bar rather than guessing:
- **Anchored PrimaryPart** (all 4 original models, initially): `Anchored = true` on the PrimaryPart silently prevents any Humanoid-driven movement — `MoveTo` calls succeed with no error, but the anchored part physically cannot move. Fix: uncheck `Anchored` on the model's PrimaryPart in Studio.
- **Welds with nil `Part0`/`Part1`** (original `ZombieBase`): every limb had a `Weld` object, but none referenced the parts they were meant to connect, so the weld did nothing — each part was an independent, unanchored body with nothing physically linking it to the torso. Root-caused by inspecting weld references directly; not fixable by unanchoring alone. Resolved by replacing the model.
- **No welds at all** (a since-replaced `ScoutRobot` candidate): parts placed adjacent to each other with zero `Weld`/`WeldConstraint`/`Motor6D` instances — visually a rig, structurally just loose parts. Resolved by replacing the model; verify any Toolbox candidate has real `Part0`/`Part1` references on every weld *before* placing it in `EnemyModels` (see `docs/getting-started.md`-style verification: dump `GetDescendants()` and check for `NIL` references).
- **Anchored non-PrimaryPart limbs with correct welds** (`GruntRobot`): welds were correctly wired, but individual limbs (not just the PrimaryPart) were still `Anchored = true`, which locks a part to the world regardless of what it's welded to. Fix: unanchor every part in the model, not just the PrimaryPart.

As of the most recent playtest: `ZombieBase`, `GruntRobot`, and `HeavyRobot` chase and move correctly (whole rig, not just the torso). `ScoutRobot` still needs a replacement model with verified, correctly-wired welds — tracked as the one remaining item before this section can be marked resolved.
